#!/usr/bin/env python3
"""Run monitor_qa evals with user_default and user_dont_think prompt settings — BASE models.

Runs 8 evals in series (4 base models × 2 prompt settings), with high
actor concurrency (500) per eval.

Usage:
    uv run python scripts/runs/run_monitor_qa_user_prompts_base.py
    uv run python scripts/runs/run_monitor_qa_user_prompts_base.py --dry-run
    uv run python scripts/runs/run_monitor_qa_user_prompts_base.py --n-samples 5
"""
...
# ---------------------------------------------------------------------------
# Base models (no model_path, no FT checkpoint)
# ---------------------------------------------------------------------------
...
def main():
    ...
    print(f"Monitor QA User-Prompt Base Evals: run_id={run_id}")
