#!/usr/bin/env python3
"""Run baseline_formatted monitor_qa mode on 8 standard models.

  - 4 base models: gpt-oss-20b, gpt-oss-120b, qwen3-8b, qwen3-32b
  - 4 stripped FT models (batch 60): ft60 variants of each
  - mode: baseline_formatted (main task only, <task_answer> format, no work)
  - monitors: metr_note (GPT-4.1), metr_note_gpt54 (GPT-5.4), main_task_grading (GPT-5.4)
  - Concurrency 1000, retry at 200 if >20% errors

Usage:
    uv run python scripts/runs/run_baseline_formatted.py --dry-run
    uv run python scripts/runs/run_baseline_formatted.py
    uv run python scripts/runs/run_baseline_formatted.py --split val
    uv run python scripts/runs/run_baseline_formatted.py --n-samples 10
"""
...
def main():
    parser = argparse.ArgumentParser(description="baseline_formatted sweep")
    ...
    log_file = RESULTS_DIR / f"baseline_formatted_{run_id}.log"
    ...
    jobs = [(m, args.split) for m in models]

    print(f"Baseline Formatted Sweep: run_id={run_id}")
    print(f"  {len(jobs)} jobs, serial execution")
    print(f"  Dataset: {DATASETS[args.split]} ({args.split} split)")
    print(f"  Mode: {MODE}")
