#!/usr/bin/env python3
"""Overnight monitor_qa eval sweep.

24 jobs: 8 models × 3 prompt settings.
  - 8 models: 4 base + 4 stripped FT (lr1e-4, step 000060)
  - 3 prompt settings: dont_think, baseline, default
  - Order: dont_think × 8, baseline × 8, default × 8
  - Concurrency: 3 jobs at a time

Usage:
    uv run python scripts/runs/overnight_monitor_qa.py
    uv run python scripts/runs/overnight_monitor_qa.py --dry-run
    uv run python scripts/runs/overnight_monitor_qa.py --n-samples 50
"""
...
# FT models (Tinker, stripped lr1e-4 step 000060)
FT_MODELS = [
    {
...
def build_jobs(run_id: str, n_samples: int | None) -> list[dict]:
    """Build job list: 3 prompt settings × 8 models = 24 jobs.

    Ordered: dont_think × 8, baseline × 8, default × 8.
    """
    jobs = []
...
def print_dry_run(run_id: str, n_samples: int | None) -> None:
    ...
    print(f"\n{'=' * 70}")
    print(f"DRY RUN — Monitor QA Overnight (run_id={run_id})")
    print(f"{'=' * 70}")
    ...
    for i, j in enumerate(jobs, 1):
        ...
        ft = f"  model_path={m['model_path']}" if m["model_path"] else ""
        print(f"  {i:>2}. {m['label']:<25} setting={setting:<12} backend={m['actor_backend']}{ft}")
    print()


def _setup_logging(run_id: str) -> None:
    log_file = RESULTS_DIR / f"overnight_monitor_qa_{run_id}.log"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Overnight monitor_qa eval sweep")
    ...
    guard = ProcessGuard(RESULTS_DIR / "logs" / "overnight_monitor_qa.pids")
    ...
    logging.info("Monitor QA overnight: %d jobs, max_concurrent=%d, run_id=%s",
    ...
    logging.info("")
    logging.info("=" * 70)
    logging.info("MONITOR QA OVERNIGHT COMPLETE — %.0fs total (%.1f hours)", elapsed, elapsed / 3600)
    logging.info("=" * 70)
