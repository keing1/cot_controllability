#!/usr/bin/env python3
"""Driver for the 4-new-model × 5000-row mixed FT run.

Same shape as ``run_mixed_5k_ft.py`` but uses
``configs/ft_mixed_5k_lr3e-5_newmodels.yaml`` and the new model roster
(DeepSeek V3.1, Kimi K2.5, Nemotron-3 Nano + Super).

Usage:
  uv run python scripts/runs/run_mixed_5k_ft_newmodels.py
  uv run python scripts/runs/run_mixed_5k_ft_newmodels.py --dry-run
  uv run python scripts/runs/run_mixed_5k_ft_newmodels.py --skip-build
  uv run python scripts/runs/run_mixed_5k_ft_newmodels.py --models deepseek-v3.1
  uv run python scripts/runs/run_mixed_5k_ft_newmodels.py --force-rebuild
"""
...
# Which models run in "thinking" mode (so we pass reasoning_effort through
# where it matters). gpt-oss uses the ``reasoning_effort`` knob, but for
# DeepSeek/Kimi/Nemotron the thinking mode is determined by the renderer
# name itself (``deepseekv3_thinking`` vs ``deepseekv3_disable_thinking``).
# So we pass ``reasoning_effort=None`` to the builder for these.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Driver: build 4 new-model mixed-5K SFT parquets + run FT + evals",
    )
