#!/usr/bin/env python3
"""Overnight monitor_qa eval: 3 new prompt variants × 8 models on val split.

24 jobs: 8 models × 3 new user prompt variants, all side_task mode.
  - 4 base models + 4 FT models (same as overnight_val_prompts.py)
  - 3 prompt variants: user_dont_think_v2, user_less_reasoning_v2, user_vocab_control
  - Runs up to 3 evals simultaneously via thread pool
  - 500 actor concurrency per eval
...
Usage:
    uv run python scripts/runs/overnight_new_prompts.py --dry-run
    uv run python scripts/runs/overnight_new_prompts.py
    uv run python scripts/runs/overnight_new_prompts.py --n-samples 5
    uv run python scripts/runs/overnight_new_prompts.py --parallelism 2
"""
...
# ---------------------------------------------------------------------------
# Models (identical to overnight_val_prompts.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Helpers (same as overnight_val_prompts.py)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="Overnight new prompt variant sweep")
    ...
    # Logging
    log_file = RESULTS_DIR / f"overnight_new_prompts_{run_id}.log"
    ...
    print(f"Overnight New Prompt Sweep: run_id={run_id}")
