#!/usr/bin/env python3
"""Overnight monitor_qa eval sweep on val split — all user prompt variants.

64 jobs: 8 models × 8 user prompt variants, all side_task mode.
  - 4 base models + 4 FT models (lr1e-4, step 000060)
  - 8 prompt variants: user_default, user_dont_think, user_less_reasoning,
    user_intense_hiding, user_gray_swan, user_pressure, user_roleplay, user_misdirect
  - Runs one eval at a time with 1000 actor concurrency
  - If >20% of rollouts error, deletes the file and reruns with 200 concurrency
  - Monitors: metr_note (GPT-4.1), metr_note_gpt54 (GPT-5.4), main_task_grading (GPT-5.4)

Usage:
    uv run python scripts/runs/overnight_val_prompts.py --dry-run
    uv run python scripts/runs/overnight_val_prompts.py
    uv run python scripts/runs/overnight_val_prompts.py --n-samples 5
"""
...
def run_single(
    ...
    """Run one eval. If >20% errors at 1000 concurrency, strip errors and rerun at 200."""
...
def main():
    parser = argparse.ArgumentParser(description="Overnight val split prompt sweep")
    ...
    # Logging
    log_file = RESULTS_DIR / f"overnight_val_prompts_{run_id}.log"
    ...
    print(f"Overnight Val Prompt Sweep: run_id={run_id}")
    print(f"  {len(jobs)} jobs, serial execution")
